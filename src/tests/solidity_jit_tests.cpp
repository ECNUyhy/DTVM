    ...
    if (Entry.is_directory()) {
      ...
      if (std::filesystem::path SolcJsonFile =
        ...
        if (std::ifstream File(TestCasesFile); File.is_open()) {
          ...
          rapidjson::Document Doc;
          Doc.ParseStream(Isw);
          // Allow individual suites to opt out while JIT support catches up.
          if (Doc.HasMember("skip") && Doc["skip"].IsBool() &&
              Doc["skip"].GetBool()) {
            if (Debug) {
              std::cout << "Skipping Solidity tests in directory: "
                        << ContractDir.string() << std::endl;
            }
            continue;
          }
